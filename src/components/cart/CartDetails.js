...
const CartDetails = () => {
  ...
  const fetchCarts = async (recalculate = false, editItem) => {
    try {
      ...
      const priceResponse = await api.get(`pricebook/priceSlabBySkus?VariantIds=${varinatSkus}`);
  ...
  const calculateTotalCount = (data) => {
    return Object.values(data).reduce((sum, variants) => sum + variants.length, 0);
  };

  const calculateComboTotalCount = (data) => { return data.length };
  ...
  const calculateTotalAvailableCount = (data) => {
    return Object.values(data).reduce((sum, variants) => {
      const publishedVariants = variants.filter(variant => variant.published === "true" && !(!variant.backOrder && variant.outOfStock) && !variant?.comboVariants?.includes(variant?.variantSku));
  ...
  useEffect(() => {
    const getTotalCountAndSelectState = (cartData, selectedItems) => {
      const totalCount = comboCart ? calculateComboAvailableCount(cartData) : calculateTotalAvailableCount(cartData);
      const isAllSelected = totalCount > 0 && totalCount === selectedItems.length;
      return { totalCount, isAllSelected };
    };
    let totalCount = 0;
    let isAllSelected = false;
    if (sampleCart) {
      const { totalCount: sampleTotal, isAllSelected: sampleIsAllSelected } = getTotalCountAndSelectState(sample, sampleSelected);
      totalCount = sampleTotal;
      isAllSelected = sampleIsAllSelected;
    } else if (wholesaleCart) {
      const { totalCount: wholesaleTotal, isAllSelected: wholesaleIsAllSelected } = getTotalCountAndSelectState(wholesale, wholesaleSelected);
      totalCount = wholesaleTotal;
      isAllSelected = wholesaleIsAllSelected;
    } else if (comboCart) {
      const { totalCount: comboTotal, isAllSelected: comboIsAllSelected } = getTotalCountAndSelectState(combo, comboSelected);
      totalCount = comboTotal;
      isAllSelected = comboIsAllSelected;
    } else if (swatchCart) {
      const { totalCount: swatchTotal, isAllSelected: swatchIsAllSelected } = getTotalCountAndSelectState(swatch, swatchSelected);
  ...
  }, [sample, wholesale, combo, swatch, sampleSelected, wholesaleSelected, comboSelected, swatchSelected, activeIndex]);
  ...
  const getItemCountAndTotalPrice = () => {
    ...
    const totalPrice = selectedItems.reduce((total, item) => {
      ...
      const price = comboCart ? item.combo.sellingPrice * (1 - discount / 100) : item.sellingPrice * (1 - discount / 100);
      const parsedPrice = isNaN(price) ? 0 : parseFloat(price).toFixed(2);
      const quantity = parseInt(comboCart ? item.combo.quantity * item.combo.comboVariants.length : item.quantity, 10) || 0;
  ...
  const filterDataByType = (orderType) => {
    ...
    Object.keys(price).forEach(key => {
      ...
      if (Array.isArray(items)) {
        const filteredItems = items.filter(item => item.orderType === orderType);
  ...
  const handleSelectAllChange = () => {
    ...
    const filterValidItems = (items) => {
      return _.filter(items, item => item.published === "true" && !(item.backOrder === false && item.outOfStock === true) && !item?.comboVariants?.includes(item?.variantSku));
    };
    let selectedItems = [];
    const itemsMap = {
      sample: sample,
  ...
  const selectedStatus = (items) => {
    ...
    if (activeIndex !== 'combo') {
      validItems = items?.filter(variant => variant.published === "true" && !(!variant.backOrder && variant.outOfStock));
    }
    let selectedList = [];
    switch (activeIndex) {
      case 'sample':
  ...
  const handleTotalChange = (items, event) => {
    ...
    const updateSelectedItems = (prevSelected, items) => {
      if (selectProductGroup === false) {
        return [
          ...prevSelected,
          ...filterValidItems(items).filter(item => !prevSelected.some(selectedItem => selectedItem.id === item.id))
        ];
      } else {
        return prevSelected.filter(selectedItem => !items.some(item => item.id === selectedItem.id));
      }
    };
    const filterValidItems = (items) => {
      return items.filter(item =>
        item.published === "true" && !(item.backOrder === false && item.outOfStock === true) && !item?.comboVariants?.includes(item?.variantSku)
      );
    };
  ...
  const handleComboTotalChange = (item, event) => {
    setComboSelected(prevSelectedItems => updateComboselection(prevSelectedItems, item));
  };
  ...
  const updateComboselection = (prevSelectedItems, ...items) => {
    ...
    const updatedItems = items.reduce((acc, item) => {
      if (!selectedSet.has(item.combo.id)) {
        ...
        const index = acc.findIndex(existingItem => existingItem.combo.id === item.combo.id);
  ...
  const handleIndividualChange = (item) => {
    switch (activeIndex) {
      case 'sample':
        setSampleSelected(prevSelectedItems => updateSelection(prevSelectedItems, item));
        break;
      case 'wholesale':
        setWholesaleSelected(prevSelectedItems => updateSelection(prevSelectedItems, item));
        break;
      case 'swatch':
        setSwatchSelected(prevSelectedItems => updateSelection(prevSelectedItems, item));
        break;
      default:
        break;
    }
  ...
  const handleEditSave = async () => {
    switch (editItem?.cartType) {
      case 'SAMPLE':
        {
          const minSampleMoq = price[editItem?.pimVariantId]?.filter(item => item.orderType === 'SAMPLE')?.[0]?.min || editItem?.sampleMoq || sampleMoq;
          const maxWholesaleMoq = price[editItem?.pimVariantId]?.filter(item => item.orderType === 'SAMPLE')?.[0]?.max || editItem?.wholesaleMoq || wholesaleMoq;
          if (editItem?.quantity < minSampleMoq || editItem?.quantity > maxWholesaleMoq) {
            setError(`Quantity must be between ${minSampleMoq} kg and ${maxWholesaleMoq} kg.`);
            return;
          }
          if (editItem?.quantity > editItem.totalStockQuantity && !editItem.backOrder) {
            setError(`Total Available Quantity is ${editItem.totalStockQuantity} kg.`);
            return;
          }
        }
        break;
      case 'WHOLESALE':
        {
          const minWholesaleMoq = price[editItem?.pimVariantId]?.filter(item => item.orderType === 'WHOLESALE')[0].min || editItem?.wholesaleMoq || wholesaleMoq;
          if (editItem.quantity < minWholesaleMoq) {
            setError(`Quantity must be at least ${minWholesaleMoq} kg.`);
            return;
          }
          if (editItem.quantity > minWholesaleMoq && (editItem?.quantity % (editItem.kgPerRoll || 20)) !== 0) {
            setError(`Quantity must be muliple of ${editItem?.kgPerRoll || 0}.`);
            return;
          }
          if (editItem?.quantity > editItem.availableQuantity && !editItem.backOrder) {
            setError(`Total Available Quantity is ${editItem.availableQuantity} kg.`);
            return;
          }
        }
        break;
      ...
      case 'SWATCH':
        {
          ...
          if (editItem.quantity < swatchMoq) {
            setError(`Quantity must be at least ${swatchMoq} units for swatch.`);
            return;
          }
        }
        break;
  ...
  const confirmRemove = async (selectedItem) => {
    try {
      await api.delete(`cart/delete?id=${comboCart ? selectedItem.combo.id : selectedItem.id}`);
      await fetchCarts();
      if (sampleCart) {
        ...
        setComboSelected(prevSelectedItems =>
          prevSelectedItems.filter(item => item.combo.id !== selectedItem.combo.id)
        );
      } else if (swatchCart) {
  ...
  const cartItems = sampleCart ? sample : wholesaleCart ? wholesale : comboCart ? combo : swatchCart ? swatch : [];
                        ...
                        <View style={styles.cartItems}>
                          {
                            _.map(cartItems, (items, articleCode) => (
                              <View key={articleCode} style={{ flexDirection: 'column' }}>
                                <CartProductSelect
                                ...
                                />
                                {
                                  comboCart ? (
                                    <>
                                      <ComboCartVariant items={items} comboSelected={comboSelected} />
                                    </>
                                  ) : (
                                    <CartVariant
                                      items={items}
