...
const ProductDetails = ({ route }) => {
  ...
  const productSpecRef = useRef(null);
  const onSpecClicked = () => {
  ...
  const fetchPimDetails = async () => {
    ...
    priceBook?.forEach((item) => {
      const samplePriceSlab = item?.priceSlabs.find(
        (priceSlab) => priceSlab?.orderType === "SAMPLE"
      );
      const wholesalePriceSlab = item?.priceSlabs.find(
        (priceSlab) => priceSlab?.orderType === "WHOLESALE"
      );
      if (samplePriceSlab) {
        setCartOptions((prev) => ({
          ...prev,
      ...
      item?.priceSlabs?.forEach((priceSlab) => {
        ...
        const discountedPrice =
          sellingPrice - (sellingPrice * priceSlab?.discount) / 100;
  ...
  const fetchColorAndImage = () => {
    const newMedia = [];
    if (pim) {
      const imageSrc = `${backendUrl}${pim?.product?.image?.replace(
        "/api",
        ""
      )}`;
      ...
      if (pim.pimVariants) {
        pim.pimVariants.forEach((variant) => {
          if (variant.image !== null) {
            const variantImageSrc = `${backendUrl}${variant.image.replace(
              "/api",
              ""
            )}`;
            newMedia.push({
    ...
    if (colourOptions.length > 0 && variantId) {
      const selectedColor = colourOptions.find(
        (item) => item.skucode === variantId
      );
      if (selectedColor) {
  ...
  useEffect(() => {
    ...
    setPriceSlab(
      priceBook?.filter((item) => item.variantId === selectedValue)[0]
        ?.priceSlabs || priceBook[0]?.priceSlabs
    );
    let backOrder = true;
    ...
    if (combo?.includes(selectedSku)) {
      ...
      backOrder = combo.every((sku) => {
        const colorOptionForSku = colorOption.find(
          (option) => option.skucode === sku
        ); // use `find` instead of `filter` for first match
        return colorOptionForSku?.backOrder || false;
      });
    } else {
      backOrder =
        _.filter(colorOption, (c) => c.id === selectedValue)[0]?.backOrder ||
        false;
    }
  ...
  const existingCart = async () => {
    if (Object.keys(pim).length > 0) {
      const existComboSwatchRes = await api.get(
        `cart/comboSwatch?id=${pim?.pimId}`
      );
  ...
  useEffect(() => {
    ...
    if (combo?.includes(selectedSku)) {
      ...
      minimumOrder <=
      cartOptions[selectedValue]?.SampleMax
    ) {
      orderType = "SAMPLE";
    }
    setCart((prev) => ({
      ...
      try {
        if (categories?.length > 0) {
          const response = await api.post(
            `pim/searchFilter?page=${0}&size=${15}&sortData=New false`,
            {
              [categories[0]?.name]: [
          ...
          const filteredProducts = pim
            ...
            .map((p) => ({
              ...p,
              pimVariants: p.pimVariants.filter((pv) => pv.status === "ACTIVE"),
            }));
    ...
    <CreateProduct.Provider value={pim}>
      ...
      >
        ...
        >
          ...
          <ProductStandardColor pimData={pim} colorOption={colorOption} />
