  ...
  const toggleWishlist = async (pimVariantId) => {
    try {
      const response = await api.post(
        `wishlist/save?pimVariantId=${pimVariantId}`
      );
      fetchWishListPImIds();
    } catch (error) {
  ...
  return (
    <SafeAreaView>
      <View style={styles.stdColorContainer}>
        <Text style={styles.stdColorHeader}>
          {_.size(pimData?.pimVariants)} Standard Colors
        </Text>
            ...
            return (
              ...
              >
                ...
                <View style={styles.colorDetails}>
                  ...
                  <Icon
                    name={
                      _.includes(pimVariantIds, item?.id) ? "heart" : "hearto"
                    }
                    size={25}
